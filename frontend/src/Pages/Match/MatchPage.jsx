					...
					{!loading && !error && results.length > 0 && (
						<div className="match-grid">
							{results.map((item) => {
								...
								return (
									<div key={recipeId} className="match-card">
												...
												<div className="match-details-column">
													...
													{Array.isArray(item.have) && item.have.length > 0 ? (
														<ul className="details-list">
															{item.have.map((h) => (
																<li key={`have-${recipeId}-${h.ingredientId}`} className="details-item">
																	<span className="details-name">{h.name}</span>
																	<span className="details-qty">
																		{h.inventoryQuantity} {h.inventoryUnit}
																		{h.requiredQuantity !== null && h.requiredQuantity !== undefined && (
																			<>
														...
														<p className="details-empty">Aucun ingrédient reconnu.</p>
													)}
												</div>

												<div className="match-details-column">
													<h4 className="details-title">❌ Manquant / à compléter</h4>
													{Array.isArray(item.missing) && item.missing.length > 0 ? (
														<ul className="details-list">
															{item.missing.map((m) => (
																<li key={`missing-${recipeId}-${m.ingredientId}`} className="details-item">
																	<span className="details-name">{m.name}</span>
																	<span className="details-qty">
																		{m.requiredQuantity !== null && m.requiredQuantity !== undefined
																			? `Requis: ${m.requiredQuantity} ${m.requiredUnit || ""}`
																			: "Quantité requise non renseignée"}
																	</span>
																	<span className="details-reason">{humanizeReason(m.reason)}</span>
																</li>
															))}
														</ul>
													) : (
